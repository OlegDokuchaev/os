...
static struct inode *my_fs_make_inode(struct super_block *sb, int mode)
{
    ...
    if (ret) {
        ...
        ret->i_atime_sec = ret->i_mtime_sec = ret->i_ctime_sec = current_time(ret);
        ret->i_ino = 1;
    }
...
static int my_fs_fill_sb(struct super_block *sb, void *data, int silent)
{
    ...
    root_inode->i_atime_sec = root_inode->i_mtime_sec = root_inode->i_ctime_sec = current_time(root_inode);
